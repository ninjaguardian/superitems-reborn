...
execute as @s[tag=!freeze_cooldown] unless score @s freeze_cooldown matches 101 if predicate superitems_reborn:is_not_sneaking run scoreboard players add @s freeze_cooldown 1
execute as @s[tag=freeze_cooldown] unless score @s freeze_cooldown matches 101 run scoreboard players add @s freeze_cooldown 1
execute as @s unless score @s freeze_cooldown matches 101 run function superitems_reborn:unicode_sender/prep_unicode/freeze_cooldown
execute as @s[scores={freeze_cooldown=68},tag=!freeze_cooldown] run tag @s add freeze_cooldown
execute as @s[scores={freeze_cooldown=101},tag=freeze_cooldown] run tag @s remove freeze_cooldown
